...
class OceanographicIntelligenceEngine:
    ...
    def _build_parameter_relationships(self) -> Dict[str, Dict]:
        ...
        return {
            ...
            'density': {
                ...
                'derived_properties': ['potential_density', 'buoyancy_frequency', 'density_anomaly'],
                'physical_processes': ['stratification', 'mixing', 'convection'],
                'typical_ranges': {'surface': (1020, 1027), 'deep': (1027, 1028)},
                'units': 'kg_m3'
            },
            'mixed_layer_depth': {
                'related_params': ['temperature', 'density', 'salinity'],
                'derived_properties': ['stratification_index', 'thermocline_depth'],
                'physical_processes': ['vertical_mixing', 'surface_forcing', 'wind_mixing'],
                'typical_ranges': {'shallow': (10, 50), 'deep': (100, 200)},
                'units': 'meters'
            }
        }
    ...
    def _identify_required_calculations(self, query_lower: str, intent: QueryIntent,
        ...
        if 'temperature' in context.parameters:
            ...
            if any(word in query_lower for word in ['surface', 'sst']):
                calculations.append('surface_temperature')
        
        if 'salinity' in context.parameters:
            if any(word in query_lower for word in ['absolute']):
                calculations.append('absolute_salinity')
            if any(word in query_lower for word in ['surface', 'sss']):
                calculations.append('surface_salinity')
        ...
        if 'stratification' in context.physical_processes:
            calculations.extend(['stratification_index', 'buoyancy_frequency'])
        
        if 'mixing' in context.physical_processes:
            calculations.extend(['mixed_layer_depth', 'richardson_number'])
    ...
    def _calculate_mixed_layer_depth(self, df: pd.DataFrame) -> pd.Series:
        ...
        try:
            ...
            for threshold in temp_criteria:
                temp_diff = np.abs(df_sorted['temperature'] - surface_temp)
                mld_idx = temp_diff[temp_diff > threshold].index
        ...
        return pd.Series([final_mld] * len(df))
    ...
    def _assess_data_quality(self, df: pd.DataFrame) -> str:
        ...
        for col in critical_params:
            if col in df.columns:
                ...
                if missing_pct > 50:
                    quality_notes.append(f"Critical: High missing data in {col} ({missing_pct:.1f}%)")
                elif missing_pct > 20:
        ...
        if not quality_notes:
            ...
            return f"Data quality is good with minor notes: {'; '.join(quality_notes)}"
        else:
            return f"Data quality assessment reveals multiple concerns: {'; '.join(quality_notes[:3])}... (review recommended)"
